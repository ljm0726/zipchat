...
public class BoardController {
	...
	@GetMapping
	public ResponseEntity<Map<String, Object>> listArticle(@ApiParam(value = "param DTO", required = true) BoardParameterDto boardParameterDto) throws Exception {
		logger.info(boardParameterDto.toString());
		logger.info("listArticle - 호출");
		
		Map<String, Object> boardListInfo = new HashMap<>();
		boardListInfo.put("boardList", boardService.listArticle(boardParameterDto));
		
		return new ResponseEntity<Map<String, Object>>(boardListInfo, HttpStatus.OK);
	}
	...
	public ResponseEntity<BoardDto> getArticle(@PathVariable("articleNo") @ApiParam(value = "얻어올 글의 글번호.", required = true) int articleNo) throws Exception {
		...
		//*************** board 가져올 때 getName을 통해 board작성자 이름 까지 가져오게 하기 ******************
		logger.info("article {}", boardService.getArticle(articleNo));
